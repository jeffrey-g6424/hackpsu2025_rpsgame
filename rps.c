...
// Only need to do this ONCE per program run to avoid repeating sequences.
// Heavily borrowed from GeeksforGeeks project idea on Rock-Paper-Scissors
void rps_seed(void) {
...
// forces it into {0,1,2}, which maps to either ROCK, PAPER, or SCISSORS.
...
// Same move        -> Tie
//
// First check the tie. Then we check all "player wins" cases.
// Otherwise CPU wins.
...
// Take the player's move as an integer, convert it to rps_move_t,
